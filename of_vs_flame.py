...
#create graph vectors
    ...
    while(P_cc_current <= P_cc_max):
        ...
        flame_temp_mw_ratio_arr.append(i[2])
...
#graph:
plt.title(label=f'Flame Temp vs Chamber Pressure for '+ ox_name + ' ' + fuel_name)
plt.xlabel('Chamber Pressure (Bar)')
plt.ylabel('Flame Temp (K)/kg)')
plt.legend()
plt.grid(True)
plt.show()
